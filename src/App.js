        ...
        fetch().then(r => {
            ...
            if ( ticks > 0 ) {
                if ( ticks === 1 ) {
                    ...
                    history[0].ticks = Math.floor(ticks / 2);
                }
            }
            this.setState({ ticks, online : (r ? 'online' : 'offline'), history })
        });
    };

    render() {
        const { ticks, online, display_mode, history } = this.state;

        return (
            <div className={'app ' + online + (display_mode === 'heart' ? ' beat' : '')}>
                ...
                <div className={'history'}>
                    {history.map(item => {
                        return <div style={{opacity: 1-history.indexOf(item)/5}} className={'history-entry'} key={history.indexOf(item)}>
                            Desconexión: {item.date.toUTCString()} - Duración: {item.ticks} seg
                        </div>
                    })}
                </div>
